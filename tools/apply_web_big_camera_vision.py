...
web_replacement = '''            settings.mixedContentMode = WebSettings.MIXED_CONTENT_NEVER_ALLOW
            ...
            setOnTouchListener { view, event ->'''
if web_anchor not in service:
    raise SystemExit('Web Big WebView settings anchor not found')
service = service.replace(web_anchor, web_replacement, 1)
...
if destroy_anchor not in service:
    raise SystemExit('WebView destroy anchor not found')
service = service.replace(destroy_anchor, destroy_replacement, 1)
...
if old_prompt not in ai:
    raise SystemExit('AI Vision instruction anchor not found')
ai = ai.replace(old_prompt, new_prompt, 1)
ai_path.write_text(ai)

print('Web Big camera permission and AI Vision detail rules updated.')
