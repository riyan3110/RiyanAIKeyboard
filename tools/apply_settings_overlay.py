...
anchor = '''    private fun addUtilityBar() {'''
insert = '''    private fun addSettingsPanel() {
...
if anchor not in text:
    raise SystemExit('utility anchor missing')
text = text.replace(anchor, insert + anchor, 1)

old = '''        if (::aiPanel.isInitialized) {
...
new = '''        if (::aiPanel.isInitialized) {
        ...
        if (aiFullscreen) {'''
if old not in text:
    raise SystemExit('root height anchor missing')
text = text.replace(old, new, 1)

old = '''    private fun openSettings() {
        runCatching {
...
if old not in text:
    raise SystemExit('open settings anchor missing')
text = text.replace(old, new, 1)

path.write_text(text)
print('Settings overlay wired into RiyanKeyboardService.kt')
