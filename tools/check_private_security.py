#!/usr/bin/env python3
"""Fail the PRIVATE build if high-risk Android capabilities are reintroduced.

This check intentionally focuses on capabilities that can make a keyboard look or behave
like a screen-control/overlay utility. It does not try to hide the IME from other apps.
"""
...
if app is None:
    ...
    if app.get(ANDROID + "usesCleartextTraffic") != "false":
        fail("usesCleartextTraffic harus false pada PRIVATE.")
    if app.get(ANDROID + "allowBackup") != "false":
        fail("allowBackup harus false pada PRIVATE.")
    if app.get(ANDROID + "networkSecurityConfig") != "@xml/network_security_config":
        fail("networkSecurityConfig PRIVATE harus aktif.")
    if app.get(ANDROID + "dataExtractionRules") != "@xml/data_extraction_rules":
        fail("dataExtractionRules PRIVATE harus aktif.")
    if app.get(ANDROID + "debuggable") == "true":
        fail("Aplikasi PRIVATE tidak boleh memaksa debuggable=true.")
...
for node in root.findall("uses-permission"):
    ...
    if name in forbidden_permissions:
        fail(f"Izin berisiko tidak boleh ada di PRIVATE: {name}")

# PRIVATE must remain a normal IME, not an AccessibilityService/overlay controller.
if "ScreenTextAccessibilityService" in manifest_text:
    fail("ScreenTextAccessibilityService tidak boleh didaftarkan di manifest PRIVATE.")
if "android.permission.BIND_ACCESSIBILITY_SERVICE" in manifest_text:
    fail("BIND_ACCESSIBILITY_SERVICE tidak boleh ada di manifest PRIVATE.")
...
if not NETWORK.exists():
    ...
    try:
        network_root = ET.fromstring(network_text)
        ...
        if cert_sources != {"system"}:
            fail("Trust anchor PRIVATE hanya boleh memakai sertifikat sistem.")
        if network_root.findall("domain-config") or network_root.findall("debug-overrides"):
            fail("PRIVATE tidak boleh memiliki pengecualian domain/debug pada network security config.")
    except ET.ParseError as exc:
        fail(f"network_security_config.xml tidak valid: {exc}")

if errors:
    print("PRIVATE security gate: GAGAL")
    for item in errors:
        print(f"- {item}")
    sys.exit(1)

print("PRIVATE security gate: OK")
