...
internal class PrivateVoiceCapture(
    ...
    override fun onResults(results: Bundle?) {
        ...
        val text = results?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)
            ?.firstOrNull { it.isNotBlank() }?.trim().orEmpty().take(2000)
        if (text.isBlank()) { fail("Tidak ada ucapan yang terbaca. Ketuk mic dan coba lagi."); return }
        cancel()
        onResult(text)
    }
    ...
    override fun onEndOfSpeech() { finish() }
