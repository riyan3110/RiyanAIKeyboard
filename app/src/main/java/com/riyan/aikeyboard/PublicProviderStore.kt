...
internal data class PublicProviderProfile(
    val id: String,
    val name: String,
    val baseUrl: String,
    val apiKey: String,
...
/** PUBLIC-only dynamic provider profiles for OpenAI-compatible HTTPS endpoints. */
internal object PublicProviderStore {
    private const val PROFILES_KEY = "public_provider_profiles_v2"
    private const val SELECTED_KEY = "public_provider_selected_v2"
    private const val MIGRATION_DONE_KEY = "public_provider_migration_done_v2"

    private const val OLD_DYNAMIC_PROFILES_KEY = "private_provider_profiles_v1"
    private const val OLD_DYNAMIC_SELECTED_KEY = "private_provider_selected_v1"
    ...
    fun providerId(baseUrl: String): String =
        "pub_" + Integer.toHexString(normalizeBaseUrl(baseUrl).lowercase().hashCode())

    fun inferName(baseUrl: String): String {
        val clean = normalizeBaseUrl(baseUrl)
        val host = runCatching { URL(clean).host }.getOrNull().orEmpty().removePrefix("www.")
    ...
    fun modelEndpoints(baseUrl: String): List<String> {
        ...
        require(base.startsWith("https://", ignoreCase = true)) {
            "Base URL PUBLIC harus memakai https://"
        }
        ...
        for (endpoint in modelEndpoints(baseUrl)) {
            val attempt = runCatching {
                ...
                try {
                    ...
                    connection.setRequestProperty("User-Agent", "AI-Ads-Keyboard/PUBLIC")
    ...
    fun load(prefs: SharedPreferences): List<PublicProviderProfile> {
        val raw = prefs.getString(PROFILES_KEY, "[]").orEmpty()
        return parseProfiles(raw)
    }

    private fun parseProfiles(raw: String): List<PublicProviderProfile> = runCatching {
        val array = JSONArray(raw)
        buildList {
            for (i in 0 until array.length()) {
                val item = array.optJSONObject(i) ?: continue
                val baseUrl = normalizeBaseUrl(item.optString("baseUrl"))
                if (baseUrl.isBlank() || !baseUrl.startsWith("https://", ignoreCase = true)) continue
    ...
    fun ensureMigrated(prefs: SharedPreferences): List<PublicProviderProfile> {
        ...
        val migratedDynamic = migrateOldDynamicProfiles(prefs)
        val imported = if (migratedDynamic.isNotEmpty()) migratedDynamic else migrateHardCodedProviders(prefs)
        if (imported.isNotEmpty()) {
            persist(prefs, imported)
            select(prefs, imported.first().id)
        }
        prefs.edit().putBoolean(MIGRATION_DONE_KEY, true).commit()
        purgeLegacyProviderState(prefs)
        return imported
    ...
    private fun migrateOldDynamicProfiles(prefs: SharedPreferences): List<PublicProviderProfile> {
        val raw = prefs.getString(OLD_DYNAMIC_PROFILES_KEY, "[]").orEmpty()
        val migrated = parseProfiles(raw)
        if (migrated.isEmpty()) return emptyList()
        val oldSelected = prefs.getString(OLD_DYNAMIC_SELECTED_KEY, null)
        val selected = migrated.firstOrNull { it.id == oldSelected } ?: migrated.first()
        val normalized = migrated.map { profile ->
            profile.copy(id = providerId(profile.baseUrl))
        }
        val selectedNewId = normalized.firstOrNull { it.baseUrl == selected.baseUrl }?.id
        if (selectedNewId != null) {
            prefs.edit().putString(SELECTED_KEY, selectedNewId).commit()
        }
        return normalized
    }

    private fun migrateHardCodedProviders(prefs: SharedPreferences): List<PublicProviderProfile> {
        data class Legacy(
        ...
        val imported = legacy.mapNotNull { old ->
            ...
            if (!baseUrl.startsWith("https://", ignoreCase = true)) return@mapNotNull null
        ...
        val selected = legacy.firstOrNull { it.providerId == selectedLegacy }?.let { old ->
        ...
        prefs.edit().putString(SELECTED_KEY, selected.id).commit()
        return imported
    }

    fun selected(
        prefs: SharedPreferences,
        profiles: List<PublicProviderProfile> = load(prefs)
    ): PublicProviderProfile? {
    ...
    fun delete(prefs: SharedPreferences, id: String): List<PublicProviderProfile> {
    ...
    fun purgeLegacyProviderState(prefs: SharedPreferences) {
        ...
        editor.remove(OLD_DYNAMIC_PROFILES_KEY)
        editor.remove(OLD_DYNAMIC_SELECTED_KEY)
        editor.commit()
    }

    private fun persist(prefs: SharedPreferences, profiles: List<PublicProviderProfile>) {
        val array = JSONArray()
