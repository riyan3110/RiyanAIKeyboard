...
class PrivateVoiceCaptureTest {
    ...
    private fun capture() = PrivateVoiceCapture(RuntimeEnvironment.getApplication(), { partials += it }, { errors += it }, { results += it })
    ...
    @Test fun cancelledSessionCannotSearchOrUpdatePanel() {
        ...
        assertTrue(results.isEmpty()); assertTrue(partials.isEmpty()); assertTrue(errors.isEmpty())
    }
    @Test fun emptyResultDoesNotSearch() {
        val c = capture()
        c.onResults(bundle(" "))
        assertTrue(results.isEmpty()); assertEquals(1, errors.size)
    }
}
