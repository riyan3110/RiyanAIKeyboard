...
class PrivateKeyboardInputTest {
    ...
    @Test fun emojiMatchesSentenceAndTrailingSpaceWithoutSubstringFalsePositive() {
        ...
        assertTrue(PrivateEmojiSuggestions.suggest("penguncian").isEmpty())
        assertTrue(PrivateEmojiSuggestions.suggest("kunci. halo").isEmpty())
        assertTrue(PrivateEmojiSuggestions.suggest("").isEmpty())
        assertEquals(listOf("🎂", "🎉", "🎁"), PrivateEmojiSuggestions.suggest("selamat ulang tahun"))
        assertTrue(PrivateEmojiSuggestions.suggest("aku minum kopi").contains("☕"))
    }
}
